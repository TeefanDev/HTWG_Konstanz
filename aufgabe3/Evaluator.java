...
public class Evaluator {
    ...
    // Zerlegt String-Eingabe in Tokens
    ...
    private static boolean shift() {
        if (stack[top] == DOLLAR && (token == KL_AUF | isVal(token))) {		        // Regel 1 der Parser-Tabelle
            ...
            doShift();
            return true;
        }  else if (top > 0 && stack[top-1] == DOLLAR && isVal(stack[top]) && (isOp(token))) {      // Regel 6
            doShift();
            return true;
        }  else if (top > 0 && stack[top-1] == KL_AUF && isVal(stack[top])
                && (token == KL_ZU || isOp(token))) {                                           // Regel 7
            doShift();
            return true;
        }  else if (top > 1 && isVal(stack[top-2]) && isOp(stack[top-1]) && isVal(stack[top])       // Regel 9
                && isOp(token)) {
            if (token == POWER && (stack[top-1] == MULT || stack[top-1] == PLUS)) {
                doShift();
                // return true; ?
    ...
    private static boolean reduce() {
        if (top > 1 && stack[top-2] == KL_AUF && isVal(stack[top-1]) && stack[top] == KL_ZU
                && (token == KL_ZU || isOp(token) || token == DOLLAR)) {                            // Regel 4
            doReduceKlValKl();
            return true;
        }  else if (top > 1 && isVal(stack[top-2]) && isOp(stack[top-1]) && isVal(stack[top])
                && (token == KL_ZU || token == DOLLAR)) {                                         // Regel 8
            doReduceValOpVal();
            return true;
        }
        return false;
    ...
    private static boolean accept() {
        return top > 0 && stack[top - 1] == DOLLAR && isVal(stack[top]) && (token == DOLLAR);       // Regel 5
    }
